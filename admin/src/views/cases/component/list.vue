 ...
 * @LastEditors: 莫卓才
 * @LastEditTime: 2021-02-20 15:33:49
-->
<template>
      ...
      <el-table-column label="分类"
        ...
        <template slot-scope="{row}">
          <template v-for="(item, index) in select">
            <span :key="index">{{ item.id==row.product_id ? item.title:''}}</span>
          </template>
        </template>
      </el-table-column>
...
export default {
  ...
  methods: {
     ...
     */
    getView (row) {
      window.open(process.env.VUE_APP_BASE_SERVER + "/cases_info/pid/25/cid/" + row.category_id + "/id/" + row.id, "blank");
    },
  }
}
</script>
