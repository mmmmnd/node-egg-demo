 ...
 * @Descripttion: 描述
 * @version: 1.0.0
 * @Date: 2020-11-12 10:41:37
 * @LastEditors: 莫卓才
 * @LastEditTime: 2020-11-19 14:55:09
-->
<template>
...
import { newsIndex, newsUpdate, newsDestroy, newsEdit, newsAdd } from '@/api/news'
  ...
  methods: {
    ...
    getList (id) {
      ...
      newsIndex(this.listQuery)
        .then(response => {
          var news = response.data.news.data;
          this.category = response.data.aboutSingleMenu;
          this.total = response.data.news.meta.total
          this.list = news.filter(newsItem => {
            var menu = this.category.filter(menuItem => newsItem.category_id === menuItem.id)
            newsItem.menu = menu[0].title;
            return newsItem
          })
     ...
     */
    getView (row) {
      window.open(process.env.VUE_APP_BASE_SERVER + "/news/pid/21/cid/" + row.category_id, "blank");
    },
    /**
     * 删除
     */
